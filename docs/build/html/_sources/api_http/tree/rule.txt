...
.. csv-table::
   ...
   "tree", "Integer", "Required", "The tree the requested rule belongs to", "", "tree", "RO", "1"
   ...
   "type", "String", "Required*", "The type of rule represented. Must be one of _________. *Required when creating a new rule.", "", "type", "RW", "METRIC"
   ...
   "displayFormat", "String", "Optional", "A display format string to alter the ''display_name'' value of the resulting branch or leaf. See ______", "", "display_format", "RW", "Port: {ovalue}"
...
A ''GET'' request requires a specific tree ID, rule level and order. Otherwise a ''400'' will be returned. To fetch all of the rules for a tree, use the ''/api/tree'' endpoint with a ''tree' value.

Example GET Query
^^^^^^^^^^^^^^^^^
::

  http://localhost:4242/api/tree/rule?tree=1&level=0&order=0
...
::
  ...
  http://localhost:4242/api/tree/rule?tree=1&level=0&order=0&type=METRIC&separator=.&method=post
...
::
  ...
  http://localhost:4242/api/tree/rule?tree=1&level=0&order=0&method=delete
